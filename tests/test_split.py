...
class TestSplit(unittest.TestCase):
    ...
    def test_process_file_not_found(self):
        ...
        self.assertTrue(
            "No such file or directory: 'asdf'" in str(context.exception))

    def test_setup_params_cli_args(self):
        args = argparse.Namespace(
            filename=self.filename, outputdir=self.outputdir)
        with patch('argparse.ArgumentParser.parse_args', return_value=args):
            ...
            self.assertTrue("Creating output xml files in " +
                            self.outputdir in output)
    ...
    def test_setup_params_invalid_params(self):
        ...
        for arg_list in [[self.filename, "asdf"], ["asdf", self.outputdir]]:
            args = argparse.Namespace(
                filename=arg_list[0], outputdir=arg_list[1])
            with patch('argparse.ArgumentParser.parse_args', return_value=args):
                with self.assertRaises(SystemExit):
                    setup_params()
                    output = self.get_stdout()
                    self.assertTrue(
                        "Invalid input or output location" in output)
